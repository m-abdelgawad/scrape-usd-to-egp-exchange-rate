...
class PostgreSQLDB:
    ...
    def update(self, update_query, values_list):
        self.cursor.execute(update_query, values_list)
        self.connection.commit()
    ...
    def get_rows_count(self, table_name):
        rows_count_query = 'SELECT COUNT(*) AS rows_count FROM {}'.format(
            table_name
        )
        self.run_query(query=rows_count_query)
        rows_count = self.fetch_results()
        return rows_count
